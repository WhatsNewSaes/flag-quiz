...
The evidence initializer prefills the `Artifact manifest` field. The full go-live gate reads that local path, writes the artifact manifest before evidence validation, and then verifies the completed evidence file, including the manifest's commit, version, build number, and signed artifact hashes. If `--skip-artifacts` is used for evidence-only review, the completed evidence file must still point to a previously generated local artifact manifest.
