...
`npm run mobile:evidence:self-test` exercises the release evidence checker against a complete synthetic signoff file. It is included in `npm run mobile:preflight` so CI catches regressions in the final evidence gate before a real release evidence file exists.
