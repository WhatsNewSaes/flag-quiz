...
The release evidence checker requires the `Artifact manifest` field to be a local JSON path so it can parse and verify the manifest's commit, version, build number, and signed artifact hashes. It also requires the final copyright holder, Apple Developer Team ID, and Google Play developer account to be filled from `docs/mobile-store-account-handoff.md`. Other screenshot, clip, QA-note, and store-console evidence fields may use external URLs or repo-relative files. External evidence URLs must point to real evidence; placeholder hosts such as `example.com`, `localhost`, `127.0.0.1`, and `.invalid` domains are rejected.

For the `Perfect Passport copied public challenge link` smoke row, paste the actual copied `https://flagarcade.com` challenge URL into the Notes cell. Localhost URLs and custom app schemes are rejected so the final evidence proves share links are public.
...
For evidence-only review when the signed builds have already been uploaded and are not available locally, make the skip explicit. The evidence file must still point to the local artifact manifest previously generated by `npm run mobile:artifacts:check`; external manifest links are not accepted by the go-live gate.
...
This checks bundle/archive structure, version metadata, Android AAB signing metadata, the archived iOS app's code-signature resources, embedded provisioning profile, and `codesign --verify`. It also writes a JSON artifact manifest with byte counts and SHA-256 hashes; keep that manifest path in the release evidence file's `Artifact manifest` field so `npm run mobile:evidence:check` can verify the manifest matches the release commit, version, and build.
