...
For launch signoff, run `npm run mobile:evidence:init -- --build 1 --owner "Release Owner"` and fill the generated `docs/release-evidence/` file with the exact build ids, devices, screenshots or clips, store-console statuses, failures, and final approval. Then run `npm run mobile:evidence:check -- --file docs/release-evidence/<release-file>.md` before final store review submission.
