...
async function main() {
  ...
  const report = [
    ...
    '- `npm run mobile:accounts:check` verifies store account handoff structure, and `npm run mobile:accounts:release` fails while final account values remain blank.',
    ...
    '- `npm run mobile:go-live:check` composes preflight, strict account handoff validation, signed artifact manifest generation, release evidence validation, and public URL checks into the final local review gate.',
