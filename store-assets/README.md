...
Package the current store assets and launch docs for handoff with `npm run package:store-submission`. The generated package is written to `dist/mobile-store-submission/`, and the shareable archive is written to `dist/flag-arcade-mobile-store-submission.zip`. Both outputs are intentionally not committed.
